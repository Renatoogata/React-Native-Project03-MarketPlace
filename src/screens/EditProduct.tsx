...
export function EditProduct() {
    ...
    async function handleUpdateProduct({ name, description, price }: FormDataProps) {
        try {
            setIsLoading(true);
            ...
            await api.put(`/products/${productId}`, { name, description, is_new, price: priceInt, accept_trade: acceptTrade, payment_methods: paymentMethods })
    ...
    async function fetchProduct() {
        try {
            ...
            setIsnew(response.data.is_new ? "new" : "used")
            setAcceptTrade(response.data.accept_trade)
            setPrice(response.data.price.toString())
            response.data.payment_methods.map((item: { key: string; }) => {
                setPaymentMethods((content) => { return [...content, item.key] })
            })
        } catch (error) {
            throw (error)
        } finally {
                ...
                isLoading ? <Loading /> :
                    ...
                    >
                        ...
                        <Controller
                            control={control}
                            name="name"
                            render={({ field: { onChange, value = product.name } }) => (
                                <Input
                        ...
                        <Controller
                            control={control}
                            name="description"
                            render={({ field: { onChange, value = product.description } }) => (
                                <Input
                        ...
                        <Controller
                            control={control}
                            name="price"
                            render={({ field: { onChange, value = price } }) => (
                                <Input
                                    placeholder="R$ 45,00"
